...
interface FormState {
  ...
  receiverStaff: string; // display name (for UI)
...
export default function ServiceTicketCreate() {
  ...
  useEffect(() => {
    ...
    const el = document.getElementById("customerName") as HTMLInputElement | null;
    el?.focus();
    ...
    fetch("http://localhost:8080/api/employees/technicians/active")
      ...
      .then((data) => {
        const items: Technician[] = Array.isArray(data)
          ? data.map((d: any) => ({
  ...
  const onChange = (
    ...
    setForm((prev) => ({ ...prev, [id]: value } as FormState));
  ...
  const selectTechnician = (t: Technician) => {
    ...
    const next = document.getElementById("receivedDate") as HTMLInputElement | null;
    next?.focus();
  };
  ...
  useEffect(() => {
    function handleClickOutside(ev: MouseEvent) {
      ...
      if (ev.target instanceof Node && !techInputRef.current.contains(ev.target as Node)) {
        setShowTechDropdown(false);
      }
    }
