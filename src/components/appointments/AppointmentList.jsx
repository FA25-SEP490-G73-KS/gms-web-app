...
function statusViToCode(status) {
    switch (status) {
        case 'Huỷ':
            return 'CANCELLED';
        case 'Đã đến':
...
export default function AppointmentList() {
    ...
    async function handleChangeStatus(id, next) {
        ...
        async function load() {
            try {
                ...
                if (!cancelled) {
                    ...
                    setAllRows(prev => [...prev, ...mapped.filter((m) => !prev.some(p => p.id === m.id))]);
