...
export default function TicketDetailPage() {
    ...
    const [dateModalAction, setDateModalAction] = useState('SAVE') // 'SAVE' | 'SEND'
    ...
    const transformQuotationItems = (items = []) => {
        ...
        // Đảm bảo luôn có hàng "Tiền công" ở cuối cùng
        const laborRow = serviceItemsResult.find(item => item.task === 'Tiền công')
        const otherServiceItems = serviceItemsResult.filter(item => item.task !== 'Tiền công')
        
        // Nếu chưa có hàng "Tiền công", tạo mới
        if (!laborRow) {
            otherServiceItems.push({
                id: `labor-${Date.now()}`,
            ...
            // Nếu đã có, đẩy xuống cuối
            otherServiceItems.push(laborRow)
        }

        return { replacementItems, serviceItemsResult: otherServiceItems }
    }
    ...
    const buildQuotationItemsPayload = () => {
        const partItems = replaceItems.map(item => {
            ...
            return {
                ...
                priceQuotationItemId: (item.priceQuotationItemId && typeof item.priceQuotationItemId === 'number') ? item.priceQuotationItemId : null,
        ...
        if (response && response.result) {
            ...
            if (normalizedData.priceQuotation) {
                if (Array.isArray(normalizedData.priceQuotation.items)) {
                    ...
                    setReplaceItems([])
                    // Luôn có hàng "Tiền công" mặc định
                    setServiceItems([{
                        id: `labor-${Date.now()}`,
                        priceQuotationItemId: null,
                        task: 'Tiền công',
    ...
    const addServiceItem = () => {
        // Tách hàng "Tiền công" ra khỏi danh sách
        const laborRow = serviceItems.find(item => item.task === 'Tiền công')
        const otherItems = serviceItems.filter(item => item.task !== 'Tiền công')
        
        // Thêm hàng mới vào danh sách các hàng khác
        ...
        // Đẩy hàng "Tiền công" xuống cuối nếu có
        if (laborRow) {
            newItems.push(laborRow)
        }
    ...
    const deleteServiceItem = async (id, priceQuotationItemId) => {
        // Không cho phép xóa hàng "Tiền công"
        const itemToDelete = serviceItems.find(item => item.id === id)
        if (itemToDelete && itemToDelete.task === 'Tiền công') {
            message.warning('Không thể xóa hàng "Tiền công"')
            return
        }
        ...
        if (priceQuotationItemId) {
            ...
            // Nếu không có priceQuotationItemId, chỉ xóa khỏi state (item mới chưa lưu)
            // Tách hàng "Tiền công" ra và đảm bảo nó ở cuối
            const laborRow = serviceItems.find(item => item.task === 'Tiền công')
            const otherItems = serviceItems.filter(item => item.id !== id && item.task !== 'Tiền công')
            
            if (laborRow) {
                setServiceItems([...otherItems, laborRow])
            } else {
                setServiceItems(otherItems)
            }
        }
    }
    ...
    const sendQuotationToCustomer = async () => {
        ...
        if (!expectedDate) {
            message.error('Vui lòng chọn ngày dự đoán nhận xe trước khi gửi báo giá.')
            return
        }
        ...
        try {
            ...
            // Cập nhật ngày dự đoán giao xe trước khi gửi báo giá
    ...
    const handleSendToCustomer = () => {
        // Kiểm tra cơ bản và mở popup ở chế độ GỬI BÁO GIÁ
        if (!ticketData?.priceQuotation?.priceQuotationId) {
            message.error('Không tìm thấy ID báo giá. Vui lòng tạo báo giá trước.')
            return
        }

        setDateModalAction('SEND')
        setShowDateModal(true)
    }
    ...
    const handleExportPDF = async () => {
        // Kiểm tra kỹ priceQuotation và items trước khi gọi API
        if (!ticketData?.priceQuotation?.priceQuotationId) {
            message.error('Không tìm thấy ID báo giá. Vui lòng tạo báo giá trước.')
            return
        }

        // Kiểm tra xem priceQuotation có items không (backend cần items để export)
        if (!ticketData.priceQuotation.items || !Array.isArray(ticketData.priceQuotation.items) || ticketData.priceQuotation.items.length === 0) {
            message.error('Báo giá chưa có dữ liệu. Vui lòng thêm linh kiện hoặc dịch vụ trước khi xuất PDF.')
            return
        }

        // Filter ra mục "Tiền công" vì backend không xử lý được mục này khi export PDF
        // Chỉ kiểm tra các items có priceQuotationItemId hợp lệ (không phải "Tiền công")
        const validItems = ticketData.priceQuotation.items.filter(item => {
            // Loại bỏ mục "Tiền công" - kiểm tra nhiều trường có thể chứa "Tiền công"
            const itemName = item.itemName || item.task || item.work || item.serviceName || item.partName || item.description || ''
            if (itemName === 'Tiền công' || itemName.includes('Tiền công')) {
                return false
            }
            // Chỉ lấy các items có priceQuotationItemId hợp lệ (số nguyên)
            return item.priceQuotationItemId != null && typeof item.priceQuotationItemId === 'number'
        })

        if (validItems.length === 0) {
            message.error('Báo giá chưa có dữ liệu hợp lệ. Vui lòng thêm linh kiện hoặc dịch vụ (không phải "Tiền công") trước khi xuất PDF.')
            return
        }

        // Cảnh báo nếu có mục "Tiền công" (backend không xử lý được mục này khi export PDF)
        const hasLaborItem = ticketData.priceQuotation.items.some(item => {
            const itemName = item.itemName || item.task || item.work || item.serviceName || item.partName || item.description || ''
            return itemName === 'Tiền công' || itemName.includes('Tiền công')
        })
        
        if (hasLaborItem) {
            // Hiển thị cảnh báo nhưng vẫn cho phép export (vì đã có items hợp lệ khác)
            console.warn('Mục "Tiền công" sẽ không được bao gồm trong file PDF do backend không xử lý được.')
        }
                        ...
                        onChange={(option) => {
                            ...
                            const existingItem = replaceItems.find(item =>
                                ...
                                (String(item.category) === String(value) ||
                                    (updates.partId && item.partId && String(item.partId) === String(updates.partId)))
                            )
  ...
  const serviceColumns = [
    ...
    return (
      ...
      `}</style>
            <div style={{ padding: '24px', background: '#ffffff', minHeight: '100vh', maxWidth: '100%' }}>
            ...
            >
                ...
                <Button
                    type="primary"
                    block
                    onClick={dateModalAction === 'SEND' ? sendQuotationToCustomer : confirmSendQuote}
                    style={{ background: '#22c55e', borderColor: '#22c55e', height: '40px' }}
                >
                    {dateModalAction === 'SEND' ? 'Gửi báo giá' : 'Lưu'}
                </Button>
