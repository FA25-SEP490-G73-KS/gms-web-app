  ...
  const columns = [
    ...
    },
    {
      ...
      render: (status, record) => {
        ...
        // Nếu đã hủy, chỉ hiển thị "Hủy" và disable
        // Nếu chưa hủy, hiển thị trạng thái hiện tại và chỉ cho phép chọn "Hủy"
        const currentStatusOption = allStatusOptions.find(opt => opt.value === statusKey) || { value: statusKey, label: status, color: config.color }
        const statusOptions = isCancelled 
          ? [{ value: 'CANCELLED', label: 'Hủy', color: '#ef4444' }]
          : [
              currentStatusOption, // Hiển thị trạng thái hiện tại
              { value: 'CANCELLED', label: 'Hủy', color: '#ef4444' } // Thêm option "Hủy"
            ]
        ...
        return (
          <select
            onChange={(e) => {
              ...
              if (newStatus && newStatus !== statusKey && newStatus === 'CANCELLED') {
            ...
            style={{
              color: config.color,
              opacity: (isUpdating || isCancelled) ? 0.6 : 1,
