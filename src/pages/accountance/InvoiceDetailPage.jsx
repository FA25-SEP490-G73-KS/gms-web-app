...
import { Card, Button, Table, message, Spin, Input, Row, Col, Modal, Tabs } from 'antd'
...
export default function InvoiceDetailPage() {
  ...
  const defaultPayOSConfig = {
    RETURN_URL: `${window.location.origin}/accountance/payments/${id}`,
  ...
  const payOSConfig = React.useMemo(() => {
    if (!paymentData?.paymentUrl) return defaultPayOSConfig
    const returnUrl = `${window.location.origin}/accountance/payments/${id}`
  ...
  useEffect(() => {
    if (paymentData?.paymentUrl && paymentMethod === 'QR' && payOSConfig && !payOSInitialized) {
      const timer = setTimeout(() => {
        const element = document.getElementById('payos-checkout-container')
        if (element) {
          try {
            console.log('Opening PayOS checkout with URL:', paymentData.paymentUrl)
            console.log('Element found:', element)
            setPayOSInitialized(true)
            open()
          } catch (error) {
            console.error('Error opening PayOS checkout:', error)
            message.error('Không thể mở giao diện thanh toán PayOS: ' + error.message)
            setPayOSInitialized(false)
          }
        } else {
          console.error('PayOS container element not found')
        }
      }, 800)
      return () => clearTimeout(timer)
    }
  }, [paymentData?.paymentUrl, paymentMethod, payOSConfig, open, payOSInitialized])
  ...
  const quotationItems = invoiceData?.serviceTicket?.priceQuotation?.items || []
  const totalMerchandise = quotationItems
    .filter(item => item.itemType === 'PART')
    .reduce((sum, item) => sum + (item.totalPrice || item.quantity * item.unitPrice || 0), 0)
  ...
  const quotationColumns = [
    ...
    },
    {
      ...
      key: 'categoryName',
      render: (text, record) => text || record.partName || record.name || record.itemName || 'N/A'
    },
    {
      title: 'Số lượng',
      dataIndex: 'quantity',
    ...
    },
    {
      ...
      align: 'right',
      render: (_, record) => {
        const total = record.totalPrice || (record.quantity || 1) * (record.unitPrice || 0)
    ...
    return (
      <AccountanceLayout>
        <div style={{ padding: '24px', textAlign: 'center' }}>
          ...
          <Button onClick={() => navigate('/accountance/payments')}>Quay lại</Button>
        </div>
      </AccountanceLayout>
    )
  }
  ...
  const serviceTicketCode = invoiceData?.serviceTicket?.serviceTicketCode || 'N/A'
        ...
        <Row gutter={24}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <span style={{ fontWeight: 600 }}>{formatCurrency(totalMerchandise)}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span>Tiền công:</span>
              <span style={{ fontWeight: 600 }}>{formatCurrency(totalLabor)}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <span style={{ fontWeight: 600, color: '#ef4444' }}>-{formatCurrency(depositReceived)}</span>
            </div>
            <div style={{ 
              display: 'flex', 
            ...
            <div style={{ marginTop: '8px' }}>
              ...
              <span style={{ fontWeight: 600 }}>{numberToWords(finalAmount)}</span>
            </div>
          </div>
        </Card>
        ...
        <Card style={{ borderRadius: '12px', borderTop: '4px solid #CBB081', marginBottom: '24px' }}>
          ...
          <div style={{ paddingLeft: '20px' }}>
            <ul style={{ margin: 0, padding: 0, listStyle: 'disc', color: '#374151' }}>
              ...
              <li style={{ marginBottom: '8px' }}>
                Không áp dụng bảo hành nếu xe tự ý sửa tại nơi khác, tai nạn hoặc ngập nước.
              </li>
              <li>
                ...
                {!paymentData ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        status={depositAmount && Number(depositAmount) > finalAmount ? 'error' : ''}
                        style={{
                          height: '40px',
                          borderRadius: '8px',
                          borderColor: depositAmount && Number(depositAmount) > finalAmount ? '#ef4444' : '#d1d5db',
                          fontSize: '14px'
                        }}
                      />
                      {depositAmount && Number(depositAmount) > finalAmount && (
                        <div style={{ 
                          ...
                          Số tiền đặt cọc không được vượt quá {formatCurrency(finalAmount)}
                        </div>
                      )}
                    </div>
                    <div>
                    ...
                    <div style={{ display: 'flex', gap: '12px', marginTop: '8px' }}>
                      ...
                      <Button
                        ...
                        disabled={paymentLoading || !depositAmount || Number(depositAmount) <= 0 || Number(depositAmount) > finalAmount}
                        style={{
                          flex: 1,
                          background: '#22c55e',
                          borderColor: '#22c55e',
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    <div style={{ 
                      ...
                      <span style={{ fontWeight: 700, fontSize: '18px', color: '#111' }}>THANH TOÁN</span>
                    </div>
                    ...
                    <Tabs
                      ...
                      items={[
                            ...
                            <div style={{ position: 'relative', width: '100%', height: '100%' }}>
                              {!payOSInitialized && (
                                <div style={{ 
                                  ...
                                  <Spin size="large" />
                                  <p style={{ marginTop: '12px' }}>Đang tải giao diện thanh toán...</p>
                                </div>
                              )}
                              <iframe
                                ...
                                scrolling="no"
                                onLoad={() => {
                                  console.log('PayOS iframe loaded')
                                  setPayOSInitialized(true)
                                }}
                                onError={(e) => {
                                  console.error('Error loading PayOS iframe:', e)
                                  message.error('Không thể tải giao diện thanh toán')
                                }}
                              />
                            </div>
                          ) : (
                        ...
                        },
                        {
                          key: 'CASH',
                          label: 'Tiền mặt',
                          children: paymentData && paymentMethod === 'CASH' ? (
                            <div style={{ padding: '20px 0', textAlign: 'center' }}>
                              <div style={{ marginBottom: '20px' }}>
                                <p style={{ fontSize: '16px', color: '#6b7280', marginBottom: '12px' }}>Số tiền thanh toán</p>
                                <div style={{ fontSize: '24px', fontWeight: 700, color: '#2563eb' }}>
                                  {formatCurrency(paymentData.amount || depositAmount)}
                                </div>
                              </div>
                              <Button
                                type="primary"
                            ...
                            <div style={{ textAlign: 'center', padding: '40px 0' }}>
                              <Spin size="large" />
                              <p style={{ marginTop: '12px', color: '#6b7280' }}>Đang tạo giao dịch thanh toán tiền mặt...</p>
                            </div>
                          )
                        }
                      ]}
